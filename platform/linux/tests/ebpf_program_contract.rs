...
fn task27_requires_separate_policy_free_cgroup_egress_program() {
    let root = repo_root();
    let manifest = fs::read_to_string(root.join("platform/linux/ebpf/focus-egress-ebpf/Cargo.toml"))
        .expect("Task 27 eBPF crate manifest is missing");
    let program = fs::read_to_string(root.join("platform/linux/ebpf/focus-egress-ebpf/src/main.rs"))
    ...
    for marker in [
        ...
        assert!(manifest.contains(marker), "eBPF manifest is missing {marker}");
    }
    ...
    for marker in [
        ...
        assert!(workflow.contains(marker), "eBPF build workflow is missing {marker}");
    }
}
