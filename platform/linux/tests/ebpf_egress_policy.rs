...
fn daemon_rule_encoding_is_stable_and_protocol_aware() {
    let tcp = Ipv4EgressRule::new(Ipv4Addr::LOCALHOST, 8080, EgressProtocol::Tcp).unwrap();
    let udp = Ipv4EgressRule::new(Ipv4Addr::LOCALHOST, 8080, EgressProtocol::Udp).unwrap();
    let other_port = Ipv4EgressRule::new(Ipv4Addr::LOCALHOST, 8081, EgressProtocol::Tcp).unwrap();
