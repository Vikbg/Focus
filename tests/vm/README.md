...
bash tests/vm/run-qemu.sh privilege-gate
```
...
`guest-runner.sh` refuses to perform any privileged fixture unless `systemd-detect-virt --vm` confirms that it is running in a virtual machine. Root-only operations such as reboot, suspend, user creation, nftables probing, daemon restart, fanotify permission control, temporary PAM and sudoers changes, and typed privilege-broker execution happen only inside that disposable guest.
...
- `privilege-gate`: create an isolated password-authenticated fixture user, cache a global sudo timestamp, prove the cached ticket authorizes unrestricted sudo before arming, arm the production privilege guard, prove the same cached ticket plus shell, service-control, nftables, and interpreter sudo bypasses fail, prove the typed `DockerStop` broker action can stop only the fixed `docker.service` fixture, then disarm, authenticate again, and prove sudo availability is restored. `DockerStart` remains fail closed for Task 21 because starting a rootful Docker daemon can expose a root-equivalent control surface; a safe start design belongs to the Task 22 broker review.
